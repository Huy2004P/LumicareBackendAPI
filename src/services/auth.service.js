...
class AuthService {
  // Hàm này sẽ tạo cả accessToken và refreshToken cùng lúc
  ...
  // Hàm login sẽ kiểm tra tài khoản, sau đó trả về thông tin người dùng cùng với accessToken và refreshToken
  async login(email, password) {
    const user = await userRepo.findByEmail(email);
    if (!user) throw new Error("Email không tồn tại trên hệ thống!");
    if (user.active === 0) {
      throw new Error("Tài khoản của bạn hiện đang bị tạm khóa. Vui lòng liên hệ quản trị viên để được hỗ trợ!");
    }
  ...
  // Hàm này sẽ nhận vào refreshToken, kiểm tra tính hợp lệ và nếu hợp lệ sẽ tạo mới accessToken và refreshToken
  async refreshToken(token) {
    if (!token) throw new Error("Token trống!");
    const decoded = verifyRefreshToken(token); 
    return this.generateTokens({ id: decoded.id, role: decoded.role });
  }
  // Hàm logout sẽ xóa refreshToken khỏi client (thường là xóa cookie hoặc localStorage), nhưng ở đây chúng ta chỉ trả về thông báo thành công
  async logout() { return { success: true, message: "Đã đăng xuất" }; }
  // Hàm này sẽ tạo mã OTP, lưu vào database cùng với thời gian hết hạn, sau đó gửi mã OTP qua email cho người dùng
  ...
  // Hàm này sẽ kiểm tra mã OTP, nếu hợp lệ thì cho phép người dùng đặt lại mật khẩu mới
  ...
  // Hàm này sẽ được gọi sau khi OTP đã được xác thực thành công, nó sẽ hash mật khẩu mới và cập nhật vào database, sau đó gửi email thông báo cho người dùng về việc thay đổi mật khẩu
